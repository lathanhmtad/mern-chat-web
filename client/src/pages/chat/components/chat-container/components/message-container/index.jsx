...
const MessageContainer = () => {
  ...
  const renderDMMessages = message =>
    <div className={`${message.sender === selectedChatData._id ? 'text-left' : 'text-right'}`}>
      {message.messageType === 'text' && (
        <div className={`${message.sender !== selectedChatData._id ? 'bg-[#8417ff]/5 text-[#8417ff]/90 border-[#8417ff]/50' : 'bg-[#2a2b33]/5 text-white/80 border-[#ffffff]/20'} border inline-block p-4 rounded my-1 max-w-[50%] break-words`}>{message.content}</div>
      )}
      {
        ...
        <div className={`${message.sender !== selectedChatData._id ? 'bg-[#8417ff]/5 text-[#8417ff]/90 border-[#8417ff]/50' : 'bg-[#2a2b33]/5 text-white/80 border-[#ffffff]/20'} border inline-block p-4 rounded my-1 max-w-[50%] break-words`}>
          {checkIfImage(message.fileUrl) ? <div className='cursor-pointer'>
            ...
            }} src={`${HOST}/${message.fileUrl}`} height={300} width={300} />
            {/* <img src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRrTFrhr_-pYR74jUgOy7IerAoHAX3zPIZZcg&s' height={300} width={300} /> */}
      ...
      <div className='text-sx text-gray-600'>{moment(message.timestamp).format('LT')}</div>
    </div>
  ...
  const renderChannelMessage = (message) => {
    ...
    return (
      <div className={`mt-5  ${message.sender._id !== userInfo.id ? 'text-left' : 'text-right'}`}>
        {message.messageType === 'text' && (
          <div className={`${message.sender._id !== userInfo.id ? 'bg-[#8417ff]/5 text-[#8417ff]/90 border-[#8417ff]/50' : 'bg-[#2a2b33]/5 text-white/80 border-[#ffffff]/20'} border inline-block p-4 rounded my-1 max-w-[50%] break-words ml-9`}>{message.content}</div>
        )}
        {
          message.messageType === 'file' &&
            ...
            <Avatar className='h-8 w-8 rounded-full overflow-hidden'>
              ...
              <AvatarFallback
                className={`uppercase h-8 w-8 text-lg 
                    flex items-center justify-center rounded-full ${getColor(message.sender.color)}`}
              >
  ...
  return (
    <div className='flex-1 overflow-y-auto scrollbar-hidden p-4 px-8 md:w-[65vw] lg:w-[70vw] xl:w-[80vw] w-full'>
      ...
      <div ref={scrollRef} />
      {
        showImage && <div className="fixed z-[1000] top-0 left-0 h-[100vh] w-[100vw] flex items-center justify-center backdrop-blur-0 flex-col">
          <div>
